 ...
 * formatDate(date1, 'yyyyq季'); // '2020春季'
 *
 * // week start
 ...
 *
 * // season start
 * formatDate(new Date('2020-01-12'), 'q'); // '春'
 * formatDate(new Date('2020-02-12'), 'q'); // '春'
 * formatDate(new Date('2020-03-13'), 'q'); // '春'
 * formatDate(new Date('2020-04-14'), 'q'); // '夏'
 * formatDate(new Date('2020-05-15'), 'q'); // '夏'
 * formatDate(new Date('2020-06-16'), 'q'); // '夏'
 * formatDate(new Date('2020-07-17'), 'q'); // '秋'
 * formatDate(new Date('2020-08-18'), 'q'); // '秋'
 * formatDate(new Date('2020-09-18'), 'q'); // '秋'
 * formatDate(new Date('2020-10-18'), 'q'); // '冬'
 * formatDate(new Date('2020-11-18'), 'q'); // '冬'
 * formatDate(new Date('2020-12-18'), 'q'); // '冬'
 *
 * // 自定义季节名
 * formatDate(new Date('2020-01-12'), 'q', { seasonText: ['spring'] }); // 'spring'
 *
 * // 自定义星期名字
 * const weekText = ['sunday', 'monday'];
 * formatDate(new Date('2020-01-12'), 'w', { weekText }); // 'sunday'
 *
 * @param [formular="yyyy-MM-dd hh:mm:ss"] 格式，默认：'yyyy-MM-dd hh:mm:ss'
 * @param date 日期
 * @param options 可选选项
...
export function formatDate(
  ...
  const o: Record<string, Function> = {
    ...
    q: () => {
      ...
      return opt.seasonText[q];
    },
    'S+': () => date.getMilliseconds(), //毫秒
    w: () => opt.weekText[date.getDay()], //周
  };
  if (/(y+)/.test(formular)) {
    const y = RegExp.$1;
  ...
  for (const k in o) {
    if (new RegExp('(' + k + ')').test(formular)) {
      ...
      formular = formular.replace(s1, v.padStart(s1.length, '0'));
    }
  }
  return formular;
}
